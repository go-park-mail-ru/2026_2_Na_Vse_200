...
Проект модели для ревью: каталог и загрузка музыки, плейлисты, избранное, чарты, история прослушивания и синхронизация плеера. Основные данные хранятся в PostgreSQL, сессии и текущее состояние плеера - в Redis, файлы - в S3.
...
В [диаграммах](er.md) `PK` обозначает первичный ключ, `FK` - внешний ключ PostgreSQL, `UK1` - альтернативный уникальный ключ. Все поля с одной пометкой `UK1` внутри одной сущности образуют один ключ; например, в `media_file` это `(bucket, object_key)`. Использована нотация Mermaid `classDiagram` для отображения сущностей, атрибутов без типов и кратностей; блоки не обозначают классы приложения. Пунктирные связи между хранилищами проверяет backend.

В альбоме один трек встречается один раз; номер трека уникален внутри альбома. В плейлисте повторы трека разрешены, но позиции уникальны. В чарте уникальны и трек, и позиция внутри чарта; одинаковое количество прослушиваний у разных треков допускается. `streams_count` хранит зафиксированный результат подсчёта принятых событий прослушивания за период чарта. Поэтому количество прослушиваний само по себе не определяет позицию.
...
История: новый запуск трека вошедшим пользователем после подтверждения начала воспроизведения создаёт `listening_event`. Повтор трека создаёт новое событие; пауза, продолжение, перемотка и передача того же запуска другому устройству не создают дублей. `id` события совпадает с `playback_id` запуска и защищает от повторной доставки; пользователь определяется сервером из сессии. Старые события не перезаписываются; снятие трека с публикации сохраняет его запись и ссылки истории.

Для ментора: подходит ли журнал прослушиваний для требования историчности? Сейчас он хранит события, но не предыдущие значения названия и других атрибутов трека. Согласование этого решения ещё необходимо.

WebSocket: запуск, пауза, перемотка и смена трека на одном устройстве обновляют общий плеер на остальных устройствах аккаунта. Предлагается одно активное устройство со звуком; остальные показывают состояние и могут управлять им. Backend проверяет сессию, атомарно меняет очередь и состояние, рассылает новую ревизию и отклоняет устаревшие команды. Аудио передаётся из S3, а не по WebSocket.
...
- `playback_queue_item`: {account_id, position} -> item_id, track_id; позиция уникальна внутри очереди аккаунта

Память backend:

- `player_connection`: {device_id} -> session_id, connection; это активное подключение плеера, а не постоянная таблица устройств

`playback_state.current_item_id` вместе с `account_id` указывает на элемент очереди того же аккаунта. `active_device_id` указывает на его активное подключение; `playback_id` - на запуск, для которого после подтверждения создаётся событие истории. Право управлять плеером прекращается при завершении сессии. После переподключения клиент получает актуальное состояние; после утраты состояния Redis воспроизведение останавливается, история PostgreSQL сохраняется. Срок жизни состояния и очереди согласуется, чтобы не оставлять ссылок на исчезнувшие элементы.
