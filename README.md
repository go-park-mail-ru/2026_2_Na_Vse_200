...
Чтобы не выставлять переменные руками в каждом терминале, есть скрипт: он подхватывает их
из `.env` и запускает команду.

```
cp .env.example .env     # один раз, дальше правите под себя
./scripts/run.sh                    # запустить сервер с настройками из .env
